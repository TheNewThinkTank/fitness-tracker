<script>
	...
	async function fetchData(endpoint, setter) {
		try {
			...
			setter([]); // Ensure a default empty array or object to avoid issues
		}
	}
	...
	function showWorkoutDetails(date) {
		resetState();
		selectedDate = date;
		fetchData(`dates/${date}`, (data) => {
			workoutDetails = data || {}; // Ensure workoutDetails is always an object
		});
	}
</script>
