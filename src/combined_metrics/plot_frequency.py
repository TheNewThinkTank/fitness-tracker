...
def main() -> None:
    ...
    workouts = np.random.poisson(lam=3, size=len(dates))
    ...
    plt.plot(data["Date"], data["Rolling_Avg"], label="4-Week Rolling Average", color="red", linewidth=2)
    ...
    sns.heatmap(pivot, annot=True, fmt="d", cmap="coolwarm", cbar_kws={"label": "Total Workouts"})
    ...
    sns.barplot(x="Year", y="Workouts", data=yearly_data, hue="Year", legend=False)
    ...
    plt.figure(figsize=(14, 5))
    plt.plot(data["Date"], data["Cumulative_Workouts"], label="Cumulative Workouts", color="green")
